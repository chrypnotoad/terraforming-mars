export const paths = {
  ADMIN: 'admin',
  API_CLONEABLEGAME: 'api/cloneablegame',
  API_CREATEGAME: 'api/creategame',
  API_DISCORD_INVITE: 'api/discord/invite',
  API_GAME: 'api/game',
  API_GAME_HISTORY: 'api/game/history',
  API_GAME_LOGS: 'api/game/logs',
  API_GAMES: 'api/games',
  API_HEAP_SNAPSHOT: 'api/heapsnapshot',
  API_IPS: 'api/ips',
  API_LEGACY_CAMPAIGNS: 'api/legacy-campaigns',
  API_METRICS: 'api/metrics',
  API_LOGOUT: 'api/logout',
  API_PLAYER: 'api/player',
  API_PROFILE: 'api/profile',
  API_PROFILE_CLAIM: 'api/profile/claim',
  API_PROFILES: 'api/profiles',
  API_SPECTATOR: 'api/spectator',
  API_STATS: 'api/stats',
  API_WAITING_FOR: 'api/waitingfor',
  AUTH_DISCORD_CALLBACK: 'auth/discord/callback',
  AUTH_DISCORD_START: 'auth/discord/start',
  AUTOPASS: 'autopass',
  CARDS: 'cards',
  GAME: 'game',
  GAMES_OVERVIEW: 'games-overview',
  HELP: 'help',
  LOAD: 'load',
  LOAD_GAME: 'load_game',
  LEGACY_CAMPAIGNS: 'legacy',
  LOGIN: 'login',
  NEW_GAME: 'new-game',
  PLAYER: 'player',
  PLAYER_INPUT: 'player/input',
  PROFILE: 'profile',
  RESET: 'reset',
  SPECTATOR: 'spectator',
  THE_END: 'the-end',
} as const;
